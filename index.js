...
// POST /api/self-score – Spieler trägt eigenen Score ein (einmalig pro Spiel)
app.post("/api/self-score", async (req, res) => {
  const scoringRow = await db.queryOne("SELECT value FROM settings WHERE key = ?", ["scoring_open"]);
  if (scoringRow?.value === "false") return res.status(403).json({ error: "Scoring ist aktuell deaktiviert." });
  ...
  const existing = await db.queryOne(
    "SELECT score FROM scores WHERE player_id = ? AND station = ?",
    [player.id, st]
  );
  if (existing) return res.status(409).json({ error: "Du hast für dieses Spiel bereits Punkte eingetragen." });
