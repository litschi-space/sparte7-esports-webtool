...
export default function App() {
  ...
  const [selfScoreGame, setSelfScoreGame] = useState(null);
  const [selfScoreInput, setSelfScoreInput] = useState("");
  const [selfScoreError, setSelfScoreError] = useState("");
  const [selfScoreSaved, setSelfScoreSaved] = useState(false);
  ...
  const selfScoreSubmit = async () => {
    if (!selfScoreGame || !registeredAs) return;
    const val = parseInt(selfScoreInput);
    if (isNaN(val) || val < 0) { setSelfScoreError("Bitte einen gültigen Score eingeben."); return; }

    const res = await fetch("/api/self-score", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ gamertag: registeredAs, station: selfScoreGame.id, score: val }),
    });

    if (!res.ok) {
      const err = await res.json();
      setSelfScoreError(err.error || "Fehler beim Eintragen.");
      return;
    }

    await loadData();
    setSelfScoreSaved(true);
    setSelfScoreError("");
    setTimeout(() => {
      setSelfScoreGame(null);
      setSelfScoreInput("");
      setSelfScoreSaved(false);
      setSelfScoreOpen(false);
    }, 2500);
  };
              ...
              {me ? (() => {
                ...
                <div style={{ maxWidth: 500, margin: "36px auto 0", padding: "24px 28px", background: myTeam ? myTeam.bg : "rgba(0,255,65,0.06)", border: `1px solid ${myTeam ? myTeam.color + "44" : "#00FF4133"}`, borderRadius: 14, boxShadow: `0 0 30px ${myTeam ? myTeam.color + "22" : "#00FF4120"}`, textAlign: "left" }}>
                  ...
                  <div style={{ display: "flex", gap: 20, marginTop: 20 }}>
                    ...
                    <div>
                      <div style={{ fontSize: 28, fontWeight: 900, color: "#00D4FF" }}>{myStationsDone}<span style={{ fontSize: 14, color: "#555" }}>/{STATIONS.length}</span></div>
                  ...
                  {/* Selbst-Scoring */}
                  <button
                    style={{ marginTop: 10, width: "100%", padding: "10px", background: selfScoreOpen ? (myTeam?.color || "#00FF41") : "rgba(255,255,255,0.05)", border: `1px solid ${myTeam?.color || "#00FF41"}66`, borderRadius: 8, color: selfScoreOpen ? "#000" : (myTeam?.color || "#00FF41"), fontSize: 13, fontWeight: 700, letterSpacing: 2, cursor: "pointer", transition: "all 0.15s" }}
                    onClick={() => { setSelfScoreOpen(o => !o); setSelfScoreGame(null); setSelfScoreInput(""); setSelfScoreError(""); setSelfScoreSaved(false); }}
                  >
                    🎮 PUNKTE SELBST EINTRAGEN
                  </button>

                  {selfScoreOpen && (
                    <div style={{ marginTop: 12, padding: "16px", background: "rgba(0,0,0,0.3)", border: `1px solid ${myTeam?.color || "#00FF41"}33`, borderRadius: 10 }}>
                      {selfScoreSaved ? (
                        <div style={{ textAlign: "center", padding: "12px 0" }}>
                          <div style={{ fontSize: 32, marginBottom: 6 }}>✅</div>
                          <div style={{ fontSize: 14, fontWeight: 900, color: "#00FF41", letterSpacing: 2 }}>SCORE GESPEICHERT!</div>
                        </div>
                      ) : !selfScoreGame ? (
                        <>
                          <div style={{ fontSize: 10, letterSpacing: 3, color: "#888", marginBottom: 10 }}>SPIEL AUSWÄHLEN</div>
                          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                            {STATIONS.map((st) => {
                              const alreadyScored = (scores[me.id] || {})[st.id] !== undefined;
                              return (
                                <button
                                  key={st.id}
                                  disabled={alreadyScored}
                                  style={{
                                    background: alreadyScored ? "rgba(255,255,255,0.03)" : `${st.color}18`,
                                    color: alreadyScored ? "#333" : st.color,
                                    border: `1px solid ${alreadyScored ? "rgba(255,255,255,0.06)" : st.color + "66"}`,
                                    borderRadius: 6, padding: "8px 6px", fontFamily: "'Courier New', monospace",
                                    fontSize: 11, fontWeight: 700, cursor: alreadyScored ? "not-allowed" : "pointer",
                                    textAlign: "center", letterSpacing: 0.5,
                                  }}
                                  onClick={() => { if (!alreadyScored) { setSelfScoreGame(st); setSelfScoreInput(""); setSelfScoreError(""); } }}
                                >
                                  {alreadyScored ? "✓ " : ""}{st.name}
                                </button>
                              );
                            })}
                          </div>
                          {!scoringOpen && <div style={{ fontSize: 11, color: "#FF4444", marginTop: 10, textAlign: "center" }}>⏸ Scoring aktuell pausiert</div>}
                        </>
                      ) : (
                        <>
                          <div style={{ fontSize: 10, letterSpacing: 3, color: "#888", marginBottom: 6 }}>PUNKTE FÜR</div>
                          <div style={{ fontSize: 15, fontWeight: 900, color: selfScoreGame.color, marginBottom: 12 }}>{selfScoreGame.name}</div>
                          <input
                            style={{ background: "rgba(255,255,255,0.07)", border: `2px solid ${selfScoreGame.color}66`, borderRadius: 6, padding: "12px 14px", color: selfScoreGame.color, fontFamily: "'Courier New', monospace", fontSize: 26, width: "100%", boxSizing: "border-box", outline: "none", textAlign: "center", fontWeight: 900 }}
                            type="number" min="0"
                            value={selfScoreInput}
                            onChange={(e) => setSelfScoreInput(e.target.value)}
                            placeholder="0"
                            autoFocus
                            onKeyDown={(e) => e.key === "Enter" && selfScoreSubmit()}
                          />
                          {selfScoreError && <div style={{ color: "#FF4444", fontSize: 12, marginTop: 8 }}>⚠ {selfScoreError}</div>}
                          <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                            <button
                              style={{ flex: 1, padding: "10px", background: "rgba(255,255,255,0.05)", border: "1px solid #333", borderRadius: 6, color: "#666", fontFamily: "'Courier New', monospace", fontSize: 12, cursor: "pointer" }}
                              onClick={() => { setSelfScoreGame(null); setSelfScoreError(""); }}
                            >← Zurück</button>
                            <button
                              style={{ flex: 2, padding: "10px", background: scoringOpen ? selfScoreGame.color : "#333", border: "none", borderRadius: 6, color: "#000", fontFamily: "'Courier New', monospace", fontSize: 13, fontWeight: 900, cursor: scoringOpen ? "pointer" : "not-allowed", letterSpacing: 1, opacity: scoringOpen ? 1 : 0.4 }}
                              onClick={scoringOpen ? selfScoreSubmit : undefined}
                            >✅ EINTRAGEN</button>
                          </div>
                        </>
                      )}
                ...
                <div style={{ display: "flex", justifyContent: "center", gap: 32, margin: "48px 0" }}>
                  {[["👾", players.length, "Spieler"], ["🎮", STATIONS.length, "Retrospiele"], ["🏆", leaderboard[0]?.nick || "—", "Führend"]].map(([icon, val, label]) => (
                    <div key={label} style={{ textAlign: "center" }}>
                ...
                <div style={{ marginTop: 48, maxWidth: 480, margin: "48px auto 0", background: "rgba(255,255,255,0.03)", borderRadius: 10, border: "1px solid rgba(255,255,255,0.07)" }}>
                  ...
                  {howToOpen && (
                    <div style={{ padding: "0 20px 16px" }}>
                      {["1. Registriere dich mit deinem Gamertag", "2. Spiele eine der 10 Retrospiele", "3. Melde deinen Score sofort beim Stationshelfer", "4. Spiele so viele Spiele wie möglich", "5. Wer die meisten Punkte sammeln kann, gewinnt die Ähre!"].map((t) => (
              ...
              <tbody>
                {leaderboard.map((p, i) => (
                  <tr key={p.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                    ...
                    <td style={{ padding: "8px 10px", textAlign: "right", color: "#888" }}>{p.stationCount}/{STATIONS.length}</td>
                ...
                {leaderboard.map((p, i) => {
                  ...
                  return (
                    <tr key={p.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                      ...
                      <td style={{ padding: "8px 10px", color: "#00D4FF" }}>{p.stationCount}/{STATIONS.length}</td>
                      {STATIONS.map((st) => {
